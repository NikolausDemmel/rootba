...
#
# Generate rootba configurations from a batch config file.
#
# Example:
#     ./generate-batch-config.py /path/to/folder
#
# It looks for the file named `rootba_batch_config.toml` inside the given folder.
...
def save_config(template, configs, combination, path_prefix):
    filename = os.path.join(path_prefix, "rootba_config_{}.toml".format("_".join(combination)))
...
def generate_configs(root_path, cmdline=[], overwrite_existing=False, revision_override=None):
    ...
    batch_config_path = os.path.join(root_path, "rootba_batch_config.toml")
...
def main():
    ...
    parser = argparse.ArgumentParser("Generate rootba configurations from a batch config file.")
